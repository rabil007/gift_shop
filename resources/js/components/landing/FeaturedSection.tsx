...
export function FeaturedSection({ featuredItems = [] }: { featuredItems?: FeaturedItem[] }) {
    ...
    return (
        <section className="mt-20 sm:mt-24 md:mt-32 px-4 sm:px-6 max-w-6xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 mb-6 px-2 sm:px-4">
                ...
                <Link href="/shop" className="hidden md:flex items-center gap-1 text-sm font-semibold text-[var(--landing-accent)] hover:text-[var(--landing-accent-hover)] cursor-pointer touch-target min-h-[44px]">
                    View All <ArrowUp className="h-4 w-4 rotate-45" />
                </Link>
            </div>
            <Carousel>
                {featuredItems.map((item) => (
                    <div key={item.id} data-carousel-item className="group relative w-[260px] sm:w-[280px] md:w-[320px] shrink-0 [scroll-snap-align:start]">
                        <Link href={`/item/${item.id}`} className="block">
                            ...
                            <div className="px-4">
                                ...
                                <p className="text-sm text-neutral-500 font-medium mt-1">{item.price_formatted}</p>
                            </div>
                        </Link>
                    </div>
                ))}
