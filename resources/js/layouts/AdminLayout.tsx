...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    const page = usePage();
    const { auth, name, logo } = page.props as unknown as { auth: { user: any | null }, name: string, logo: string | null };
            ...
            <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col hidden md:flex shrink-0 border-r border-slate-800">
                ...
                <div className="px-4 py-4">
                    ...
                    <nav className="flex-1 space-y-1">
                        ...
                        <Link href="/admin/enquiries" className={navLinkClass(isActive('/admin/enquiries'))}>
                            <Mail className="h-5 w-5 stroke-[1.5] group-hover:scale-110 transition-transform" />
                            Enquiries
                        </Link>
                    </nav>
